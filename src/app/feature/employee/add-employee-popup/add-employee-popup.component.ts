...
export class AddEmployeePopupComponent implements OnInit, OnDestroy {
    ...
    addEmployee(formData: any) {
        const joinedDate = moment(new Date()).format("YYYY-MM-DD");
        const employeeId = "EM" + this.getEmployeeId();
    ...
    getEmployeeId() {
        ...
        for (const emp of this.employeeList) {
            if (emp.id) employeeIds.push(emp.id.substring(2, emp.id.length));
        }
        employeeIds = employeeIds.map((x: any) => +x);
        employeeIds.sort((a: number, b: number) => a - b);
        return employeeIds.length > 0 ? employeeIds[employeeIds.length] + 1 : 1000;
    }
